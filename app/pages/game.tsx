...
const Game: BlitzPage = () => {
  ...
  const handleStop = async () => {
    ...
    if (currentUser?.error && currentUser?.error > currentError) {
      setIsNewRecord(true)
  ...
  return (
    <>
      <div className="h-2/5 flex flex-col items-center justify-end">
        {isNewRecord && <div className="text-7xl text-red-500 font-bold mb-16">New Record</div>}
        <div className="bg-[#FFEE00] h-48 w-full rounded-2xl flex items-center">
          <div className="bg-black m-4 w-full rounded-2xl">
            <p className="text-center text-[#FFEE00] text-[100px] mx-auto">{time.toFixed(2)}</p>
          </div>
        </div>
      </div>
